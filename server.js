...
function parseProyecto(p, tiposMap = null, publicMode = false) {
  ...
  try {
    ...
    if (publicMode) {
      ...
      galeria = parsed.map(img =>
        typeof img === "string"
          ? { url: img, visible: true, portada: false }
          : img
      );
    }
  } catch {
...
app.get("/api/proyectos", (req, res) => {
  db.query("SELECT * FROM proyectos ORDER BY orden", (err, rows) => {
    ...
    res.json(rows.map(p => parseProyecto(p, null, true)));
  });
});
...
app.get("/api/proyectos-listado", (req, res) => {
  db.query("SELECT id, id_categoria FROM tipos_obra", (err, tipos) => {
    ...
    db.query(
      ...
      (err, rows) => {
        if (err) return res.json([]);
        const parsed = rows.map(p => parseProyecto(p, tiposMap, true));
        res.json(parsed);
      }
    );
  });

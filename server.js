...
app.put("/api/tipos-obra/:id", (req, res) => {
  ...
  const { nombre, id_categoria, visible_en_menu } = req.body;

  db.query(
    "UPDATE tipos_obra SET id_categoria = ?, nombre = ?, visible_en_menu = ? WHERE id = ?",
    [id_categoria, nombre, visible_en_menu ? 1 : 0, id],
    (err) => {
      if (err) return res.status(500).json({ error: err.message });
...
app.delete("/api/tipos-obra/:id", (req, res) => {
  const id = parseInt(req.params.id);
  db.query("DELETE FROM tipos_obra WHERE id = ?", [id], (err) => {
    if (err) return res.status(500).json({ error: err.message });
    res.json({ success: true });
  });
});

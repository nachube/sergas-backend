...
app.get("/api/proyectos-publicos", (req, res) => {
  db.query(
    "SELECT * FROM proyectos WHERE visible = 1 ORDER BY orden",
    (err, rows) => {
      if (err) return res.json([]);
...
app.get("/api/proyectos-home", (req, res) => {
  db.query(
    "SELECT * FROM proyectos WHERE visible = 1 AND visibilidad = 'home' ORDER BY orden",
    (err, rows) => {
      if (err) return res.json([]);
...
app.get("/api/proyectos-listado", (req, res) => {
  db.query(
    "SELECT * FROM proyectos WHERE visible = 1 AND visibilidad = 'proyectos' ORDER BY orden",
    (err, rows) => {
      if (err) return res.json([]);

...
// =============================================
// DATOS DE LA EMPRESA (AdminCompanyData)
// =============================================
...
app.get("/api/company", (req, res) => {
  db.query("SELECT * FROM company_data LIMIT 1", (err, rows) => {
    ...
    if (rows.length === 0) {
      return res.json({
        phone_main: "+54 294 44425177",
        phone_whatsapp: "+5429444425177",
        email_contact: "sergas@sergas.ar",
        address: "Vice Alte. O'Connor 665 3° A, (8400) San Carlos de Bariloche, Río Negro, Argentina",
        schedule: "Lunes a Viernes 8:00 - 17:00 hs",
        footer_text_1: "Más de 40 años construyendo infraestructura de calidad para mejorar la vida de las personas.",
        footer_text_2: "Invertimos en el futuro."
      });
    }
    res.json(rows[0]);
  });
  ...
  db.query("SELECT id FROM company_data LIMIT 1", (err, rows) => {
    ...
    const query = rows.length > 0
      ? `UPDATE company_data SET 
           ...
           address = ?, schedule = ?, footer_text_1 = ?, footer_text_2 = ? 
           WHERE id = ?`
      : `INSERT INTO company_data 
           (phone_main, phone_whatsapp, email_contact, address, schedule, footer_text_1, footer_text_2) 
           VALUES (?, ?, ?, ?, ?, ?, ?)`

    const params = rows.length > 0
      ? [phone_main, phone_whatsapp, email_contact, address, schedule, footer_text_1, footer_text_2, rows[0].id]
      : [phone_main, phone_whatsapp, email_contact, address, schedule, footer_text_1, footer_text_2];

    db.query(query, params, (err) => {
      if (err) return res.status(500).json({ error: err.message });
      res.json({ success: true, message: "Datos de la empresa actualizados" });
    });
  });
});

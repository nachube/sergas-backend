...
const categoria = tiposMap
  ? categoriasIds.map(id => {
      if (typeof id === "string" && !Number.isInteger(Number(id))) return id; // ya es slug
      return tiposMap[id] || tiposMap[Number(id)] || null;
    }).filter(Boolean)
  : categoriasIds;

  // 🔹 Si hay mapa → estamos en API pública → convertir a slugs
const categoria = tiposMap
  ? categoriasIds.map(id => {
      if (typeof id === "string" && isNaN(Number(id))) return id; // ya es slug, devolver directo
      return tiposMap[id] || tiposMap[Number(id)] || null;
    }).filter(Boolean)
  : categoriasIds;
  ...
  db.query("SELECT * FROM users WHERE email = ?", [email], (err, rows) => {
    ...
    res.json({
      ...
      user: {
        ...
        nombre: user.nombre,
        activo: user.activo,
        permisos,  // ← aseguramos que llegue como objeto
        role: user.role || 'user'
      }
    });
  });
...
app.get("/api/proyectos/:id", (req, res) => {
  db.query("SELECT * FROM proyectos WHERE id = ?", [req.params.id], (err, rows) => {
    if (err || rows.length === 0) return res.status(404).json(null);
    res.json(parseProyecto(rows[0]));
  });
});
...
app.put("/api/proyectos/:id", (req, res) => {
  ...
  const dataParaUpdate = {
    ...
    visibilidad,
    // Si quieres permitir cambiar orden o visible desde el form → agrégalos aquí
  };
...
app.get("/api/company", (req, res) => {
  db.query("SELECT * FROM company_data WHERE id = 1", (err, rows) => {
    if (err) return res.status(500).json({ error: err.message });
...
app.put("/api/company", (req, res) => {
  const {
    phone_main, phone_whatsapp, email_contact,
    address, schedule, hero_text, footer_text_1, footer_text_2
  } = req.body;

  db.query(
    `UPDATE company_data SET 
      phone_main = ?, phone_whatsapp = ?, email_contact = ?,
      address = ?, schedule = ?, hero_text = ?,
      footer_text_1 = ?, footer_text_2 = ?
    WHERE id = 1`,
    [phone_main, phone_whatsapp, email_contact, address, schedule, hero_text, footer_text_1, footer_text_2],
    (err) => {
      if (err) return res.status(500).json({ error: err.message });
      res.json({ success: true });
    }
...
app.post("/api/chat", async (req, res) => {
  ...
  try {
    // 1. Cargar contenido de assistant_knowledge (textos editables desde admin)
    const [knowledge] = await db.promise().query(
      "SELECT titulo, categoria, contenido FROM assistant_knowledge WHERE activo = 1 ORDER BY orden ASC"
    );

    // 2. Cargar base_conocimiento (archivos/documentos subidos)
    const [docs] = await db.promise().query(
      "SELECT titulo, categoria, descripcion FROM base_conocimiento WHERE activo = 1 ORDER BY orden ASC"
    );

    // 3. Armar contexto como texto plano
    const bloques = [
      ...knowledge.map(k => `### ${k.titulo}${k.categoria ? ` (${k.categoria})` : ''}\n${k.contenido || ''}`),
      ...docs.map(d => `### ${d.titulo}${d.categoria ? ` (${d.categoria})` : ''}\n${d.descripcion || ''}`)
    ];

    const contexto = bloques.length > 0
      ? bloques.join('\n\n')
      : 'No hay información adicional disponible.';

    // 4. Limitar historial a últimos 10 mensajes para no exceder tokens
    ...
    const completion = await groq.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: `Sos el asistente virtual de SERGAS, una empresa argentina de infraestructura con más de 40 años de experiencia en Bariloche, Patagonia. Respondé siempre en español, de forma amable, profesional y concisa. Si no sabés algo, sugerí contactar a la empresa directamente. Usá la siguiente información como base de conocimiento:\n\n${contexto}`
        },
        ...historialReciente,

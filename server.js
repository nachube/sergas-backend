...
app.post("/api/company", (req, res) => {
  ...
  db.query("SELECT id FROM company LIMIT 1", (err, rows) => {
    if (rows.length) {
      db.query("UPDATE company SET ? WHERE id=?", [data, rows[0].id], err2 => {
        if (err2) return res.status(500).send("Error update");
        res.send("OK");
      });
    } else {
      db.query("INSERT INTO company SET ?", data, err3 => {
        if (err3) return res.status(500).send("Error insert");
        res.send("OK");
      });
    }
  });

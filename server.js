        ...
        conn.query(
          ...
          err => {
            ...
            if (completed === orden.length) {
              conn.commit(err => {
                if (err) return conn.rollback(() => { conn.release(); res.status(500).json({ error: err }); });
                conn.release();
                res.json({ success: true });
              });
            }
...
// ──────────────── CONFIG (para max_visible) ────────────────
// Asumimos una tabla config simple (creala si no existe: id INT PK, clave VARCHAR, valor TEXT)
app.get("/api/config/estadisticas_max_visible", (req, res) => {
  db.query("SELECT valor FROM config WHERE clave = 'estadisticas_max_visible' LIMIT 1", (err, rows) => {
    if (err || rows.length === 0) return res.json({ max: 6 }); // default 6 si no existe
...
app.post("/api/estadisticas", (req, res) => {
  ...
  // Obtener el próximo orden (último +1)
...
app.get("/api/config/estadisticas_max_visible", (req, res) => {
  db.query("SELECT valor FROM config WHERE clave = 'estadisticas_max_visible' LIMIT 1", (err, rows) => {
    if (err || rows.length === 0) return res.json({ max: 6 }); // default
    res.json({ max: parseInt(rows[0].valor) || 6 });
  });
});
...
// Reordenar (para drag & drop)
app.post("/api/tipos-obra-reordenar", (req, res) => {
  const { orden } = req.body; // array de ids en nuevo orden: [3,1,4,2,...]

    ...
    const result = await new Promise((resolve, reject) => {
      const stream = cloudinary.uploader.upload_stream(
        { 
          ...
          resource_type: "auto",              // importante para PDFs, Word, Excel, etc.
...
app.put("/api/company", (req, res) => {
  ...
  db.query("SELECT id FROM company_data LIMIT 1", (err, rows) => {
    ...
    const query = rows.length > 0
      ? "UPDATE company_data SET ? WHERE id = ?"
      : "INSERT INTO company_data SET ?";

    const params = rows.length > 0 ? [data, rows[0].id] : [data];

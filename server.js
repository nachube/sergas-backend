...
app.post("/api/chat", async (req, res) => {
  ...
  try {
    // 1. Cargar contexto de ambas tablas
    const [knowledge] = await db.promise().query(
      "SELECT titulo, contenido, categoria FROM assistant_knowledge WHERE activo = 1 ORDER BY orden ASC"
    );
    const [docs] = await db.promise().query(
      "SELECT titulo, descripcion, categoria FROM base_conocimiento WHERE activo = 1 ORDER BY orden ASC"
    );

    // 2. Armar contexto como texto
    const contexto = [
    ...
    const completion = await groq.chat.completions.create({
      model: "mixtral-8x7b-32768", // ejemplo de modelo Groq
      messages: [
        {
          role: "system",
          content: `Sos el asistente virtual de SERGAS, una empresa argentina de infraestructura con más de 40 años de experiencia en Bariloche. Respondé de forma amable, profesional y concisa. Usá la siguiente información como base de conocimiento:\n\n${contexto}`
        },
        ...historial,

...
app.post("/api/proyectos", (req, res) => {
  let categoriaJson;
  try {
    categoriaJson = JSON.stringify(
      Array.isArray(req.body.categoria) ? req.body.categoria : []
    );
  } catch (err) {
    categoriaJson = '[]';
    console.error('Error al stringify categoria en POST:', err, req.body.categoria);
  }
...
app.put("/api/proyectos/:id", (req, res) => {
  ...
  try {
    ...
    categoriaJson = '[]';
    console.error('Error al stringify categoria en PUT:', err, req.body.categoria);
  }
  ...
  db.query("UPDATE proyectos SET ? WHERE id = ?", [data, id], err => {
    if (err) return res.status(500).json({ error: "Error update" });
    res.send("OK");
  });
});
